...
export default function LegacyHostPage() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      try {
        ...
        console.error("[LegacyHost] Connection failed:", error);
  ...
  const startCamera = async () => {
    try {
      ...
      console.log("[LegacyHost] Camera started");
    } catch (error) {
      console.error("[LegacyHost] Camera error:", error);
  ...
  const startScreenShare = async () => {
    try {
      ...
      stream.getVideoTracks()[0].addEventListener("ended", () => {
        console.log("[LegacyHost] Screen share stopped by user");
        stopSource();
      });
      ...
      console.log("[LegacyHost] Screen share started");
    } catch (error) {
      console.error("[LegacyHost] Screen share error:", error);
  ...
  const toggleHostFlip = () => {
    ...
    if (store.roomId) {
      ...
      console.log("[LegacyHost] Sent display options:", {
        flipHorizontal: newFlipState,
      });
    }
  };
  ...
  useEffect(() => {
    if (store.roomId && remoteStream) {
      ...
      console.log("[LegacyHost] Sent frame layout settings:", settings);
    }
  ...
  useEffect(() => {
    if (remoteStream && remoteVideoRef.current) {
      ...
      console.log("[LegacyHost] Remote stream connected");
    }
  }, [remoteStream]);
  ...
  useEffect(() => {
    const handlePhotoSelectSync = (message: any) => {
      console.log("[LegacyHost] Received peer photo selection:", message);
  ...
  useEffect(() => {
    const handleGuestDisplayOptions = (message: any) => {
      console.log("[LegacyHost] Received guest display options:", message.options);
      if (message.options) {
  ...
  useEffect(() => {
    const handleSessionSettings = (message: any) => {
      console.log(
        "[LegacyHost] Received session settings broadcast from server:",
        message
      );
      if (message.settings) {
        console.log(
          "[LegacyHost] Broadcast settings - recordingDuration:",
  ...
  useEffect(() => {
    const handleVideoFrameRequest = async (message: any) => {
      console.log("[LegacyHost] Received video frame request:", message);

      if (message.selectedPhotos && message.selectedPhotos.length === selectablePhotos) {
        console.log(
          "[LegacyHost] Auto-composing video frame for photos:",
  ...
  useEffect(() => {
    const handlePhotosMerged = (message: any) => {
      console.log("[LegacyHost] Received merged photos from server:", message);
      ...
      if (message.photos && Array.isArray(message.photos)) {
        ...
        console.log(`[LegacyHost] Displayed ${mergedPhotos.length} merged photos`);
      }
    };
  ...
  const startPhotoSession = () => {
    ...
    console.log(
      "[LegacyHost] ========== PHOTO SESSION START (Individual Recording) =========="
    );
    console.log("[LegacyHost] Session settings:");
    console.log(
      "[LegacyHost]  - recordingDuration:",
      recordingDuration,
      "seconds (영상 녹화 시간 = 촬영 카운트다운 시간)"
    );
    console.log(
      "[LegacyHost]  - captureInterval:",
      captureInterval,
      "seconds (사진 촬영 후 다음 사진까지 대기 시간)"
    );
    console.log(
      "[LegacyHost] Mode: Individual segment recording (no FFmpeg splitting needed!)"
    );
    console.log("[LegacyHost] ================================================");
    ...
    console.log("[LegacyHost] Sending session settings to server:", sessionSettings);
  ...
  const takePhoto = async (photoNumber: number) => {
    ...
    console.log("[LegacyHost] ========== Taking photo", photoNumber, "==========");
    console.log("[LegacyHost] Starting individual video recording for this segment");
      ...
      try {
        await videoRecorderRef.current.startRecording(
          ...
          (blob, completedPhotoNumber) => {
            ...
            console.log(
              `[LegacyHost] ✅ Video segment ${completedPhotoNumber} recorded:`,
              {
            ...
            setRecordedSegments((prev) => {
              ...
              console.log(
                `[LegacyHost] Total segments recorded: ${newSegments.length}/8`
              );
              return newSegments;
            });
        ...
        console.log(
          `[LegacyHost] Recording started for photo ${photoNumber} (${recordingDuration}s)`
        );
      } catch (error) {
        console.error("[LegacyHost] Failed to start recording:", error);
        setCurrentlyRecording(null);
      }
    }
    ...
    setCountdown(count);
    console.log("[LegacyHost] Starting countdown from", count, "seconds");
    ...
    const interval = setInterval(() => {
      count--;
      console.log("[LegacyHost] Countdown:", count);
  ...
  const capturePhoto = async (photoNumber: number) => {
    console.log(`[LegacyHost] 📸 Capturing photo ${photoNumber}`);
    ...
    if (localCanvas && store.roomId) {
      try {
        ...
        if (photoNumber < totalPhotos) {
          console.log("[LegacyHost] Photo", photoNumber, "captured successfully");
          console.log(
            "[LegacyHost] ⏱️  Waiting",
            captureInterval,
            "seconds before next photo"
          );
          setTimeout(() => {
            console.log("[LegacyHost] Starting photo", photoNumber + 1);
            takePhoto(photoNumber + 1);
          }, captureInterval * 1000);
        } else {
          // Last photo
          console.log("[LegacyHost] ✅ Last photo captured!");
          setIsCapturing(false);
          startProcessing();
          console.log("[LegacyHost] Photo session complete, waiting for merge...");
        }
      } catch (error) {
        console.error(`[LegacyHost] Failed to upload photo ${photoNumber}:`, error);
  ...
  const handleGenerateFrame = async () => {
    ...
    try {
      ...
      console.log("[LegacyHost] Photo frame generated and downloaded");
    } catch (error) {
      console.error("[LegacyHost] Failed to generate frame:", error);
  ...
  const autoComposeAndUploadVideo = async (selectedPhotoIndices: number[]) => {
    if (!store.roomId || !store.userId) {
      console.error("[LegacyHost] Missing roomId or userId");
      return;
    }

    // Check if we have recorded segments
    if (recordedSegments.length === 0) {
      console.error("[LegacyHost] No recorded segments available");
      alert("녹화된 영상이 없습니다.");
      return;
    }
    ...
    if (selectedSegments.length !== selectablePhotos) {
      console.error(
        `[LegacyHost] Failed to find all segments (${selectedSegments.length}/${selectablePhotos})`
      );
      alert(
        `선택한 사진 중 ${
    ...
    console.log("[LegacyHost] 🚀 Auto-composing with Canvas 2D + selected layout");
    console.log(
      "[LegacyHost] Auto-composing video frame with segments:",
      selectedSegments.map((s) => s.photoNumber)
    );
    console.log("[LegacyHost] Selected layout:", store.selectedFrameLayoutId);
    ...
    try {
      ...
      if (!selectedLayout) {
        console.error("[LegacyHost] Selected layout not found:", store.selectedFrameLayoutId);
        alert("선택한 프레임 레이아웃을 찾을 수 없습니다.");
        setIsComposing(false);
        return;
      }

      console.log("[LegacyHost] Using layout for auto composition:", {
      ...
      const composedBlob = await composeVideoWithWebGL(
        ...
        (progress) => {
          ...
          console.log("[LegacyHost] Video compose progress:", progress);
        }
      );

      console.log("[LegacyHost] Composition complete, uploading to server...");
      ...
      console.log(`[LegacyHost] Uploading ${extension.toUpperCase()} video (${(composedBlob.size / 1024 / 1024).toFixed(2)} MB)`);
      ...
      console.log("[LegacyHost] Upload complete:", result);
      ...
      console.log("[LegacyHost] ✅ Video composition & upload complete:", {
      ...
      console.error("[LegacyHost] Failed to compose/upload video:", error);
      alert(
  ...
  const handleComposeVideoFrame = async () => {
    ...
    console.log("[LegacyHost] 🚀 Canvas 2D 합성 시작 + selected layout");
    console.log(
      "[LegacyHost] Composing video frame with segments:",
      selectedSegments.map((s) => s.photoNumber)
    );
    console.log("[LegacyHost] Selected layout:", store.selectedFrameLayoutId);
    ...
    try {
      ...
      console.log("[LegacyHost] Using layout for manual composition:", {
        id: selectedLayout.id,
        label: selectedLayout.label,
        slotCount: selectedLayout.slotCount,
        videoSourcesCount: videoSources.length
      ...
      setComposedVideo({ blob: composedBlob, url });
      console.log("[LegacyHost] ✅ Video composition complete:", {
        codec: composedBlob.type,
        size: `${(composedBlob.size / 1024 / 1024).toFixed(2)} MB`,
        format: composedBlob.type.includes('mp4') ? 'MP4 (H.264 Hardware)' : 'WebM (Software)',
      ...
      console.error("[LegacyHost] Failed to compose video with WebGL:", error);
      alert(
        "영상 합성에 실패했습니다. " +
          (error instanceof Error ? error.message : "")
      );
  ...
  useEffect(() => {
    if (store.peerId && localStream) {
      console.log(
        "[LegacyHost] Peer joined, waiting before creating offer:",
        store.peerId
      );
      // Wait a bit for guest to initialize their stream
      const timer = setTimeout(() => {
        console.log("[LegacyHost] Creating offer for peer:", store.peerId);
        createOffer().catch((error) => {
          console.error("[LegacyHost] Failed to create offer:", error);
        });
  ...
  useEffect(() => {
    return () => {
      console.log("[LegacyHost] Component unmounting - cleaning up resources");
      stopSource();
      if (videoRecorderRef.current) {
  ...
  useEffect(() => {
    console.log('[LegacyHost] Checking MediaRecorder codec support...');
    checkCodecSupport();
  }, []);
