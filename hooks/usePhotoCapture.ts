...
export function usePhotoCapture({ roomId, userId, selectedLayout, onFlash }: UsePhotoCaptureOptions) {
  ...
  /**
   * Capture photo locally without uploading to server
   * Returns Base64 PNG data for client-side processing
   */
  const captureLocal = useCallback(async ({
    photoNumber,
    canvasOrVideo,
    isCanvas = true
  }: CaptureLocalParams): Promise<string> => {
    // Trigger flash effect
    onFlash?.();
    ...
    const targetWidth = RESOLUTION.PHOTO_WIDTH;
    const targetHeight = RESOLUTION.PHOTO_HEIGHT;
    console.log(`[PhotoCapture] captureLocal: Using fixed resolution: ${targetWidth}x${targetHeight}`);
    ...
    console.log(`[PhotoCapture] captureLocal: Photo ${photoNumber} captured at ${targetWidth}x${targetHeight}`);
    setPhotoCount(photoNumber);
